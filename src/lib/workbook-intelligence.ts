...
export function numericValue(raw: string): number | null {
  ...
  if (!/^[+-]?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?%?$/.test(value) || /^[+-]?0\d/.test(value)) return null;
  const n = Number(value.replace(/,/g, '').replace(/%$/, '')) * (negative ? -1 : 1) / (value.endsWith('%') ? 100 : 1);
  return Number.isFinite(n) ? n : null;
}
export type ColumnProfile = { index: number; name: string; filled: number; missing: number; unique: number; formulas: number; numeric: number; sum: number | null; mean: number | null; min: number | null; max: number | null; outliers: number };
export function profileSheet(sheet: Sheet) {
  ...
  const columns: ColumnProfile[] = Array.from({ length: width }, (_, index) => {
    ...
    const numbers = values.map(numericValue).filter((v): v is number => v !== null).sort((a, b) => a - b);
    const sum = numbers.length ? numbers.reduce((a, b) => a + b, 0) : null;
    const quartile = (q: number) => { const pos = (numbers.length - 1) * q; const a = numbers[Math.floor(pos)] ?? 0; return a + ((numbers[Math.ceil(pos)] ?? a) - a) * (pos % 1); };
    const q1 = quartile(.25), q3 = quartile(.75), iqr = q3 - q1;
    return { index, name: sheet.rows[0]?.[index] || columnLabel(index), filled: filled.length, missing: values.length - filled.length, unique: new Set(filled).size, formulas: values.filter(v => v.startsWith('=')).length, numeric: numbers.length, sum, mean: sum === null ? null : sum / numbers.length, min: numbers[0] ?? null, max: numbers.at(-1) ?? null, outliers: numbers.length >= 4 ? numbers.filter(n => n < q1 - 1.5 * iqr || n > q3 + 1.5 * iqr).length : 0 };
  });
  return { rows: body.length, columns, duplicates, blankRows: sheet.rows.slice(1).length - body.length, missing: columns.reduce((n, c) => n + c.missing, 0), formulas: columns.reduce((n, c) => n + c.formulas, 0) };
}
...
export function cleanSheet(sheet: Sheet, action: CleanAction): Sheet {
  if (action === 'trim') return { ...sheet, rows: sheet.rows.map(row => row.map(v => v.startsWith('=') ? v : v.trim())) };
  const width = sheet.rows.reduce((n, r) => Math.max(n, r.length), 0);
...
export function pivotSheet(sheet: Sheet, group: number, value: number, aggregation: 'sum' | 'average' | 'count'): Sheet {
  ...
  for (const row of sheet.rows.slice(1)) {
    ...
    const n = numericValue(row[value] ?? ''); bucket.count++;
    if (n !== null) { bucket.total += n; bucket.numeric++; }
    groups.set(key, bucket);
  }
  return { name: `${sheet.name.slice(0, 22)} Summary`, rows: [[sheet.rows[0]?.[group] || 'Group', `${aggregation} of ${sheet.rows[0]?.[value] || 'Value'}`], ...Array.from(groups, ([key, b]) => [key, aggregation === 'count' ? String(b.count) : b.numeric === 0 ? '' : String(aggregation === 'average' ? b.total / b.numeric : b.total)])] };
}
...
export function workbookDiff(before: Sheet[], after: Sheet[], limit = 100) {
  const changes: CellChange[] = []; let total = 0;
  ...
  for (const name of names) {
    const a = before.find(s => s.name === name)?.rows ?? [], b = after.find(s => s.name === name)?.rows ?? [];
    for (let r = 0; r < Math.max(a.length, b.length); r++) for (let c = 0; c < Math.max(a[r]?.length ?? 0, b[r]?.length ?? 0); c++) {
      const oldValue = a[r]?.[c] ?? '', newValue = b[r]?.[c] ?? '';
      if (oldValue === newValue) continue;
      total++; if (changes.length < limit) changes.push({ sheet: name, cell: `${columnLabel(c)}${r + 1}`, before: oldValue, after: newValue });
    }
  }
...
export function workbookContext(sheets: Sheet[], activeSheet?: string, maxChars = 60000): string {
  ...
  return ordered.map((sheet, index) => {
    ...
    budget -= part.length; return part;
  }).join('\n');
}
