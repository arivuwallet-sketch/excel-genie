...
export const INTERMEDIATE_TEMPLATES: FinancialTemplate[] = [
  {
    ...
    build: () => {
      ...
      const bs = S("Balance Sheet", [
        ...
        [
          "Accounts receivable",
          ...cols.map((c) => `='Income Statement'!${c}3*'Income Statement'!$B$34/365`),
        ],
        ["Inventory", ...cols.map((c) => `=-'Income Statement'!${c}4*'Income Statement'!$B$35/365`)],
        ["Total current assets", ...cols.map((c) => `=SUM(${c}2:${c}4)`)],
        ["Net PP&E", ...cols.map((c, i) => (i === 0 ? "=8500" : `=${COL(i - 1)}6+${c}7+${c}8`))],
        ["CapEx", ...cols.map((c) => `='Income Statement'!${c}3*'Income Statement'!$B$37`)],
        ["Depreciation", ...cols.map((c) => `='Income Statement'!${c}8`)],
        ["Total assets", ...cols.map((c) => `=${c}5+${c}6`)],
        [],
        [
          "Accounts payable",
          ...cols.map((c) => `=-'Income Statement'!${c}4*'Income Statement'!$B$36/365`),
        ],
