...
import { MAX_ROWS, MAX_COLS, MAX_CELLS, parseCellAddress, validateWorkbook } from "./workbook-limits.ts";
...
export function applyOperations(current: Sheet[], ops: SheetOp[]): OpResult {
  try { validateWorkbook(current); } catch (e) { return { sheets: current, problems: [String(e)] }; }
  if (ops.length > 200) return { sheets: current, problems: ["At most 200 operations per request."] };
  let sheets: Sheet[] = current.map((s) => ({ name: s.name, rows: s.rows.map((r) => [...r]) }));
  const problems: string[] = [];

  for (const op of ops) {
    try {
      if ((op.op === "insert_rows" || op.op === "delete_rows" || op.op === "rename_sheet") && sheets.some(s => s.rows.some(r => r.some(v => v.startsWith("="))))) throw new Error("Structural edits on formula workbooks require reference translation. Use targeted edits or a separate output sheet.");
      if (op.op === "insert_rows" || op.op === "delete_rows") {
        const target = findSheet(sheets, op.sheet);
        if (!Number.isInteger(op.atRow) || op.atRow < 1 || op.atRow > (target?.rows.length ?? 0) + (op.op === "insert_rows" ? 1 : 0)) throw new Error("Invalid row position.");
        if (op.op === "delete_rows" && (!Number.isInteger(op.count) || op.count < 1 || op.atRow - 1 + op.count > (target?.rows.length ?? 0))) throw new Error("Invalid row count.");
        if (op.op === "insert_rows" && (target?.rows.length ?? 0) + op.rows.length > MAX_ROWS) throw new Error("Row limit exceeded.");
      }
      ...
      if (op.op === "set_range") {
        ...
        if (!start || start.row + op.values.length > MAX_ROWS || start.col + width > MAX_COLS) throw new Error("Range exceeds supported bounds.");
      }
    switch (op.op) {
      case "create_sheet": {
      ...
      case "set_cells": {
        ...
        for (const c of op.cells) {
          ...
          if (target.rows.length * Math.max(widthOf(target), pos.col + 1) > MAX_CELLS) throw new Error("Cell limit exceeded.");
      ...
      case "set_range": {
        ...
        if (target.rows.length * Math.max(widthOf(target), neededWidth) > MAX_CELLS) throw new Error("Cell limit exceeded.");
      ...
      case "insert_rows": {
        ...
        if (width > MAX_COLS || width * (target.rows.length + op.rows.length) > MAX_CELLS) throw new Error("Cell limit exceeded.");
